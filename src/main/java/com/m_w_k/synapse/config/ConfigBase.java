...
public abstract class ConfigBase {
    ...
    protected int depth;
    protected List<CValue<?, ?>> allValues = new ObjectArrayList<>();
    protected List<ConfigBase> children = new ObjectArrayList<>();
    ...
    protected interface IValueProvider<V, T extends ForgeConfigSpec.ConfigValue<V>>{
        T provide(ForgeConfigSpec.Builder builder);
    }
    ...
    public class CValue<V, T extends ForgeConfigSpec.ConfigValue<V>> {
        @Nullable protected ForgeConfigSpec.ConfigValue<V> value;
        protected String name;
        private final IValueProvider<V, T> provider;

        protected CValue(String name, IValueProvider<V, T> provider, String... comment) {
            this.name = name;
            this.provider = builder -> {
                addComments(builder, comment);
                return provider.provide(builder);
            };
            allValues.add(this);
        }
        ...
        public void register(ForgeConfigSpec.Builder builder) {
            value = provider.provide(builder);
        }
